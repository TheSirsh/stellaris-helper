function summarizeArray(sumArray: Array<Array<string>>) {
  ...
  for (let i = 0; i < sumArray.length; i++) {
    if (sumArray[i]) {
      ...
      if (sumArray[i][1][sumArray[i][1].length - 1] === "%") { isProsect = true; } 
        ...
        if (isProsect) {
          if (sumArray[i][1][0] === "-") { num = -parseInt(sumArray[i][1].slice(1, -1)); isMinus = true; } 
            else { num = parseInt(sumArray[i][1].slice(0, -1)); }
          ...
          if (sumArray[i][1][0] === "-") { num = -parseInt(sumArray[i][1].slice(1)); isMinus = true; } 
            else { num = parseInt(sumArray[i][1]); }
