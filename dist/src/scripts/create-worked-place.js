...
function createTraitList(parentElemnet, arrayOfTraits, cssClass) {
    ...
    for (let i = 0; i < arrayOfTraits.length; i++) {
        ...
        if (arrayOfTraits[i].border) {
            createNewImageElement(`${cssClass}trait-container-image_${arrayOfTraits[i].border}`, traitContainer, arrayOfTraits[i].icon, arrayOfTraits[i].nameEN);
        }
        else {
            createNewImageElement(`${cssClass}trait-container-image`, traitContainer, arrayOfTraits[i].icon, arrayOfTraits[i].nameEN);
        }
...
function createDescrBlock(cssClass, workedplaceText, isImage) {
    ...
    if (isImage) {
        createNewImageElement(`${cssClass}descr-image`, mainSectionTraitDescription, workedplaceText["image"], workedplaceText["imageAltEN"]);
    }
