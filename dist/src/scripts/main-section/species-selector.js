...
function selectSpecies(i) {
    const speciesActive = document.querySelector(".workedplace__trait-container_active");
    if (speciesActive) {
        speciesActive.classList.toggle("workedplace__trait-container_active");
    }
    const mainSectionTraitList = document.querySelectorAll(".main-section__species-list");
    if (mainSectionTraitList) {
        mainSectionTraitList.forEach(element => { element.remove(); });
    }
    if (i !== 0) {
        const mainSectionWorkedPlace = document.querySelector(".main-section__workedplace_species");
        if (i !== 1) {
