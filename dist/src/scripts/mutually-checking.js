...
function mutuallyChecking(i, arr, active, sectionName) {
    const traitDescriptionTitle = document.querySelector(`.${sectionName}descr-title`);
    traitDescriptionTitle.textContent = arr[i].nameEN;
    if (arr[i].exclude !== undefined) {
        const traitActive = document.querySelectorAll(`.${sectionName}trait-container_active`);
    ...
    const notActiveArr = document.querySelectorAll(`.${sectionName}trait-container`);
    ...
    const traitDescriptionTrait = document.querySelector(`.${sectionName}descr-text`);
